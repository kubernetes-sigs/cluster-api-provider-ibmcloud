...
func TestResourceName(t *testing.T) {
	tests := []struct {
		...
		{
			// suffix "subnet-us-south-1" = 17 chars; separator = 1; maxBase = 63-17-1 = 45.
			// Build a cluster name where base[:45] ends with '-'.
			// "my-cluster-" is 11 chars; 45/11=4 rem 1 → index 44 = position 0 in "my-cluster-" = 'm'. No hyphen.
			// Use "mycluster-" (10 chars); 45%10=5 → index 44 = 5th char of "mycluster-" = 's'. No hyphen.
			// Easiest: craft name as strings.Repeat("x", 44) + "-" + "extra" = 49 chars.
			// base[:45] = "xxxx...x-" (44 x's + '-'), TrimRight strips the '-' → base = "x"*44.
			// Result = "x"*44 + "-" + "subnet-us-south-1" = 44+1+17 = 62 chars.
			name:        "trailing hyphen at boundary stripped, no double-hyphen",
			clusterName: strings.Repeat("x", 44) + "-extra",
			rt:          ResourceTypeSubnet,
			qualifier:   "us-south-1",
			want:        strings.Repeat("x", 44) + "-subnet-us-south-1",
		},
		{
			// suffix "vpc" = 3 chars; separator = 1; maxBase = 63-3-1 = 59.
			// Craft name where base[:59] ends with '-':
			// strings.Repeat("x", 58) + "-" + "yz" = 61 chars. base[:59] = "x"*58 + '-'. Strip → "x"*58.
			// Result = "x"*58 + "-vpc" = 62 chars.
			name:        "hyphen exactly at truncation boundary is removed",
			clusterName: strings.Repeat("x", 58) + "-yz",
			rt:          ResourceTypeVPC,
			want:        strings.Repeat("x", 58) + "-vpc",
		},
	}
