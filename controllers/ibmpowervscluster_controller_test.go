	...
	testCases := []struct {
		...
		},
		{
			...
			expectedError: kerrors.NewAggregate([]error{
				fmt.Errorf("failed to reconcile VPC: %w", fmt.Errorf("error checking VPC: %w", fmt.Errorf("failed to get VPC: error fetching VPC details with name: %w", errors.New("vpc not found")))),
	...
	}{
		{
			...
			conditions: capiv1beta1.Conditions{
				capiv1beta1.Condition{
					...
					Message:            "error checking VPC: failed to get VPC: error fetching VPC details with name: vpc not found",
				},
			},
		},
		{
