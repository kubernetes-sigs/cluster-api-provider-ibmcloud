...
import (
	...
	"sigs.k8s.io/cluster-api-provider-ibmcloud/pkg/cloud/scope/powervs"
	...
	t.Run("Reconciling IBMPowerVSImage ", func(t *testing.T) {
		...
		t.Run("Reconciling an image import job", func(t *testing.T) {
			...
			t.Run("When failed to get the import job using jobID", func(_ *testing.T) {
				mockpowervs.EXPECT().GetJob(gomock.Any(), gomock.AssignableToTypeOf(jobID)).Return(nil, errors.New("Error finding the job"))
			...
			t.Run("When import job status is queued", func(_ *testing.T) {
				mockpowervs.EXPECT().GetJob(gomock.Any(), gomock.AssignableToTypeOf(jobID)).Return(job, nil)
				result, err := reconciler.reconcile(ctx, powervsCluster, imageScope)
				g.Expect(err).To(BeNil())
				g.Expect(imageScope.IBMPowerVSImage.Finalizers).To(ContainElement(infrav1.IBMPowerVSImageFinalizer))
				g.Expect(imageScope.IBMPowerVSImage.Status.ImageState).To(BeEquivalentTo(infrav1.PowerVSImageStateQueued))
				expectImageConditionsV1beta2(g, imageScope.IBMPowerVSImage, []conditionAssertion{{infrav1.ImageImportedV1Beta2Condition, corev1.ConditionFalse, clusterv1.ConditionSeverityInfo, string(infrav1.PowerVSImageStateQueued)}})
				g.Expect(result.RequeueAfter).To(Not(BeZero()))
			})
			t.Run("When importing image is still in progress", func(_ *testing.T) {
				job.Status.State = ptr.To("")
				mockpowervs.EXPECT().GetJob(gomock.Any(), gomock.AssignableToTypeOf("job-1")).Return(job, nil)
				result, err := reconciler.reconcile(ctx, powervsCluster, imageScope)
				g.Expect(err).To(BeNil())
				g.Expect(imageScope.IBMPowerVSImage.Finalizers).To(ContainElement(infrav1.IBMPowerVSImageFinalizer))
				g.Expect(imageScope.IBMPowerVSImage.Status.ImageState).To(BeEquivalentTo(infrav1.PowerVSImageStateImporting))
				expectImageConditionsV1beta2(g, imageScope.IBMPowerVSImage, []conditionAssertion{{infrav1.ImageImportedV1Beta2Condition, corev1.ConditionFalse, clusterv1.ConditionSeverityInfo, infrav1.ImageNotReadyV1Beta2Reason}})
				g.Expect(result.RequeueAfter).To(Not(BeZero()))
			})
			t.Run("When import job status is failed", func(_ *testing.T) {
				...
				expectImageConditionsV1beta2(g, imageScope.IBMPowerVSImage, []conditionAssertion{{infrav1.ImageImportedV1Beta2Condition, corev1.ConditionFalse, clusterv1.ConditionSeverityError, infrav1.ImageImportFailedV1Beta2Reason}})
				g.Expect(result.RequeueAfter).To(Not(BeZero()))
			})
			...
			t.Run("When import job status is completed and fails to get the image details", func(_ *testing.T) {
				...
				mockpowervs.EXPECT().GetImage(gomock.Any(), gomock.AssignableToTypeOf("capi-image-id")).Return(nil, errors.New("Failed to the image details"))
				result, err := reconciler.reconcile(ctx, powervsCluster, imageScope)
				g.Expect(err).To(Not(BeNil()))
				g.Expect(result.RequeueAfter).To(BeZero())
				expectImageConditionsV1beta2(g, imageScope.IBMPowerVSImage, []conditionAssertion{{conditionType: infrav1.ImageImportedV1Beta2Condition, status: corev1.ConditionTrue}})
				g.Expect(imageScope.IBMPowerVSImage.Finalizers).To(ContainElement(infrav1.IBMPowerVSImageFinalizer))
			})
			image := &models.Image{
				Name:    ptr.To("capi-image"),
			...
			t.Run("When import job status is completed and image state is queued", func(_ *testing.T) {
				mockpowervs.EXPECT().GetJob(gomock.Any(), gomock.AssignableToTypeOf("job-1")).Return(job, nil)
				mockpowervs.EXPECT().ListImages(gomock.Any()).Return(images, nil)
				mockpowervs.EXPECT().GetImage(gomock.Any(), gomock.AssignableToTypeOf("capi-image-id")).Return(image, nil)
				result, err := reconciler.reconcile(ctx, powervsCluster, imageScope)
				g.Expect(err).To(BeNil())
				g.Expect(imageScope.IBMPowerVSImage.Finalizers).To(ContainElement(infrav1.IBMPowerVSImageFinalizer))
			...
			t.Run("When import job status is completed and image state is undefined", func(_ *testing.T) {
				...
				g.Expect(err).To(BeNil())
				g.Expect(imageScope.IBMPowerVSImage.Finalizers).To(ContainElement(infrav1.IBMPowerVSImageFinalizer))
				expectImageConditionsV1beta2(g, imageScope.IBMPowerVSImage, []conditionAssertion{{infrav1.ImageReadyV1Beta2Condition, corev1.ConditionUnknown, clusterv1.ConditionSeverityNone, infrav1.ImageStateUnknownV1Beta2Reason}})
				g.Expect(result.RequeueAfter).To(Not(BeZero()))
			})
			t.Run("When import job status is completed and image state is active", func(_ *testing.T) {
				...
				result, err := reconciler.reconcile(ctx, powervsCluster, imageScope)
				g.Expect(err).To(BeNil())
				expectImageConditionsV1beta2(g, imageScope.IBMPowerVSImage, []conditionAssertion{{conditionType: infrav1.ImageReadyV1Beta2Condition, status: corev1.ConditionTrue}})
				g.Expect(imageScope.IBMPowerVSImage.Finalizers).To(ContainElement(infrav1.IBMPowerVSImageFinalizer))
				g.Expect(imageScope.IBMPowerVSImage.Status.ImageState).To(Equal(infrav1.PowerVSImageStateACTIVE))
				g.Expect(result.RequeueAfter).To(BeZero())
			})
		})
	})
...
func TestIBMPowerVSImageReconciler_delete(t *testing.T) {
	...
	t.Run("Reconcile deleting IBMPowerVSImage ", func(t *testing.T) {
		...
		t.Run("Should fail to delete the import image job", func(t *testing.T) {
			...
			imageScope.IBMPowerVSImage.Finalizers = []string{infrav1.IBMPowerVSImageFinalizer}
			mockpowervs.EXPECT().DeleteJob(gomock.Any(), gomock.AssignableToTypeOf("job-1")).Return(errors.New("Failed to deleted the import job"))
			_, err := reconciler.reconcileDelete(ctx, imageScope)
			g.Expect(err).To(Not(BeNil()))
			g.Expect(imageScope.IBMPowerVSImage.Finalizers).To(ContainElement(infrav1.IBMPowerVSImageFinalizer))
		})
		t.Run("Should delete the import image job", func(t *testing.T) {
			...
			imageScope.IBMPowerVSImage.Status.JobID = "job-1"
			imageScope.IBMPowerVSImage.Finalizers = []string{infrav1.IBMPowerVSImageFinalizer}
			mockpowervs.EXPECT().DeleteJob(gomock.Any(), gomock.AssignableToTypeOf("job-1")).Return(nil)
			_, err := reconciler.reconcileDelete(ctx, imageScope)
			g.Expect(err).To(BeNil())
			g.Expect(imageScope.IBMPowerVSImage.Finalizers).To(Not(ContainElement(infrav1.IBMPowerVSImageFinalizer)))
		})
		t.Run("Should fail to delete the image using ID when delete policy is not to retain it", func(t *testing.T) {
			...
			imageScope.IBMPowerVSImage.Finalizers = []string{infrav1.IBMPowerVSImageFinalizer}
			mockpowervs.EXPECT().DeleteImage(gomock.Any(), gomock.AssignableToTypeOf("capi-image-id")).Return(errors.New("Failed to delete the image"))
			_, err := reconciler.reconcileDelete(ctx, imageScope)
			g.Expect(err).To(Not(BeNil()))
			g.Expect(imageScope.IBMPowerVSImage.Finalizers).To(ContainElement(infrav1.IBMPowerVSImageFinalizer))
		})
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.powervsImage != nil {
				...
				if tc.expectError {
					...
					err = testEnv.Get(ctx, key, image)
					g.Expect(err).To(BeNil())
					//TODO: Need to add more tests for different conditions like WorkspaceReady
					expectImageConditions(g, image, []metav1.Condition{{Type: infrav1.IBMPowerVSImageReadyCondition, Status: metav1.ConditionUnknown, Reason: infrav1.IBMPowerVSImageReadyUnknownReason}})
				} else {
					g.Expect(err).To(BeNil())
				}
			}
...
func expectImageConditions(g *WithT, m *infrav1.IBMPowerVSImage, expected []metav1.Condition) {
	g.Expect(len(m.Status.Conditions)).To(BeNumerically(">=", len(expected)))
	for _, c := range expected {
		actual := conditions.Get(m, infrav1.IBMPowerVSImageReadyCondition)
...
func expectImageConditionsV1beta2(g *WithT, m *infrav1.IBMPowerVSImage, expected []conditionAssertion) {
	g.Expect(len(m.Status.Conditions)).To(BeNumerically(">=", len(expected)))
	for _, c := range expected {
