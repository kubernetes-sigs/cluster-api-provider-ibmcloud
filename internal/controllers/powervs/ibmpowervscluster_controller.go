...
func (r *IBMPowerVSClusterReconciler) reconcile(ctx context.Context, clusterScope *powervsscope.ClusterScope) (ctrl.Result, error) {
	...
	if needsRequeue {
		log.Info("PowerVS or VPC infrastructure components are still provisioning, requeuing before Transit Gateway")
		return ctrl.Result{RequeueAfter: 30 * time.Second}, nil
	}

	log.Info("Reconciling transit gateway")
	if requeue, err := clusterScope.ReconcileTransitGateway(ctx); err != nil {
		cond, leg := r.buildConds(infrav1.TransitGatewayReadyCondition, infrav1.TransitGatewayReadyV1Beta2Condition, metav1.ConditionFalse, infrav1.TransitGatewayNotReadyReason, infrav1.TransitGatewayReconciliationFailedV1Beta2Reason, err.Error())
		conditions.Set(clusterScope.IBMPowerVSCluster, cond)
		deprecatedv1beta1conditions.Set(clusterScope.IBMPowerVSCluster, leg)
	...
	cond, leg := r.buildConds(infrav1.TransitGatewayReadyCondition, infrav1.TransitGatewayReadyV1Beta2Condition, metav1.ConditionTrue, infrav1.TransitGatewayReadyReason, "", "")
	conditions.Set(clusterScope.IBMPowerVSCluster, cond)
	deprecatedv1beta1conditions.Set(clusterScope.IBMPowerVSCluster, leg)
	...
	if clusterScope.IBMPowerVSCluster.Spec.COSInstance.Type != "" {
		...
		if err := clusterScope.ReconcileCOSInstance(ctx); err != nil {
			c, l := r.buildConds(infrav1.COSInstanceReadyCondition, infrav1.COSInstanceReadyV1Beta2Condition, metav1.ConditionFalse, infrav1.COSInstanceNotReadyReason, infrav1.COSInstanceReconciliationFailedV1Beta2Reason, err.Error())
			conditions.Set(clusterScope.IBMPowerVSCluster, c)
			deprecatedv1beta1conditions.Set(clusterScope.IBMPowerVSCluster, l)
			return reconcile.Result{}, fmt.Errorf("failed to reconcile COS instance: %w", err)
		}
		c, l := r.buildConds(infrav1.COSInstanceReadyCondition, infrav1.COSInstanceReadyV1Beta2Condition, metav1.ConditionTrue, infrav1.COSInstanceReadyReason, "", "")
		conditions.Set(clusterScope.IBMPowerVSCluster, c)
		deprecatedv1beta1conditions.Set(clusterScope.IBMPowerVSCluster, l)
	}
...
func (r *IBMPowerVSClusterReconciler) reconcilePowerVSResources(ctx context.Context, clusterScope *powervsscope.ClusterScope) componentResult {
	...
	if requeue, err := clusterScope.ReconcileWorkspace(ctx); err != nil {
		c, l := r.buildConds(infrav1.WorkspaceReadyCondition, infrav1.ServiceInstanceReadyV1Beta2Condition, metav1.ConditionFalse, infrav1.WorkspaceNotReadyReason, infrav1.ServiceInstanceReconciliationFailedV1Beta2Reason, err.Error())
		res.conditions = append(res.conditions, c)
		res.legacy = append(res.legacy, l)
	...
	c, l := r.buildConds(infrav1.WorkspaceReadyCondition, infrav1.ServiceInstanceReadyV1Beta2Condition, metav1.ConditionTrue, infrav1.WorkspaceReadyReason, "", "")
	res.conditions = append(res.conditions, c)
	res.legacy = append(res.legacy, l)

	// At this point, the Workspace ID must exist, so the PowerVS client must be initialized.
	if clusterScope.IBMPowerVSClient == nil {
		res.err = errors.New("fatal: PowerVS client is nil but workspace is ready")
		return res
	}

	log.Info("Reconciling network")
	if networkActive, err := clusterScope.ReconcileNetwork(ctx); err != nil {
		c, l := r.buildConds(infrav1.NetworkReadyCondition, infrav1.NetworkReadyV1Beta2Condition, metav1.ConditionFalse, infrav1.NetworkNotReadyReason, infrav1.NetworkReconciliationFailedV1Beta2Reason, err.Error())
		res.conditions = append(res.conditions, c)
		res.legacy = append(res.legacy, l)
		...
		c, l = r.buildConds(infrav1.NetworkReadyCondition, infrav1.NetworkReadyV1Beta2Condition, metav1.ConditionTrue, infrav1.NetworkReadyReason, "", "")
		res.conditions = append(res.conditions, c)
		res.legacy = append(res.legacy, l)
	}
...
func (r *IBMPowerVSClusterReconciler) reconcileVPCResources(ctx context.Context, clusterScope *powervsscope.ClusterScope) componentResult {
	...
	if requeue, err := clusterScope.ReconcileVPC(ctx); err != nil {
		c, l := r.buildConds(infrav1.VPCReadyCondition, infrav1.VPCReadyV1Beta2Condition, metav1.ConditionFalse, infrav1.VPCNotReadyReason, infrav1.VPCReconciliationFailedV1Beta2Reason, err.Error())
		res.conditions = append(res.conditions, c)
		res.legacy = append(res.legacy, l)
	...
	c, l := r.buildConds(infrav1.VPCReadyCondition, infrav1.VPCReadyV1Beta2Condition, metav1.ConditionTrue, infrav1.VPCReadyReason, "", "")
	res.conditions = append(res.conditions, c)
	res.legacy = append(res.legacy, l)
	...
	if requeue, err := clusterScope.ReconcileVPCSubnets(ctx); err != nil {
		c, l := r.buildConds(infrav1.VPCSubnetReadyCondition, infrav1.VPCSubnetReadyV1Beta2Condition, metav1.ConditionFalse, infrav1.VPCSubnetNotReadyReason, infrav1.VPCSubnetReconciliationFailedV1Beta2Reason, err.Error())
		res.conditions = append(res.conditions, c)
		res.legacy = append(res.legacy, l)
	...
	c, l = r.buildConds(infrav1.VPCSubnetReadyCondition, infrav1.VPCSubnetReadyV1Beta2Condition, metav1.ConditionTrue, infrav1.VPCSubnetReadyReason, "", "")
	res.conditions = append(res.conditions, c)
	res.legacy = append(res.legacy, l)
	...
	if err := clusterScope.ReconcileVPCSecurityGroups(ctx); err != nil {
		c, l := r.buildConds(infrav1.VPCSecurityGroupReadyCondition, infrav1.VPCSecurityGroupReadyV1Beta2Condition, metav1.ConditionFalse, infrav1.VPCSecurityGroupReconciliationFailedReason, infrav1.VPCSecurityGroupReconciliationFailedV1Beta2Reason, err.Error())
		res.conditions = append(res.conditions, c)
		res.legacy = append(res.legacy, l)
		res.err = fmt.Errorf("failed to reconcile VPC security groups: %w", err)
		return res
	}
	c, l = r.buildConds(infrav1.VPCSecurityGroupReadyCondition, infrav1.VPCSecurityGroupReadyV1Beta2Condition, metav1.ConditionTrue, infrav1.VPCSecurityGroupReadyReason, "", "")
	res.conditions = append(res.conditions, c)
	res.legacy = append(res.legacy, l)
	...
	if loadBalancerReady, err := clusterScope.ReconcileLoadBalancers(ctx); err != nil {
		c, l := r.buildConds(infrav1.VPCLoadBalancerReadyCondition, infrav1.LoadBalancerReadyV1Beta2Condition, metav1.ConditionFalse, infrav1.VPCLoadBalancerNotReadyReason, infrav1.LoadBalancerReconciliationFailedV1Beta2Reason, err.Error())
		res.conditions = append(res.conditions, c)
		res.legacy = append(res.legacy, l)
		...
		c, l = r.buildConds(infrav1.VPCLoadBalancerReadyCondition, infrav1.LoadBalancerReadyV1Beta2Condition, metav1.ConditionTrue, infrav1.VPCLoadBalancerReadyReason, "", "")
		res.conditions = append(res.conditions, c)
		res.legacy = append(res.legacy, l)
	}
...
// buildConds generates both a metav1.Condition and a legacy v1beta1 Condition simultaneously.
// reason is used for the modern condition; legacyReason is used for the legacy condition.
func (r *IBMPowerVSClusterReconciler) buildConds(condType string, legacyType clusterv1.ConditionType, status metav1.ConditionStatus, reason, legacyReason, msg string) (metav1.Condition, *clusterv1.Condition) {
