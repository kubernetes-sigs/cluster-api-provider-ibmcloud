...
func TestReconcileVPCSubnets(t *testing.T) {
	...
	t.Run("When VPCSubnets are set in spec", func(t *testing.T) {
		...
		instance, err := clusterScope.ReconcileVPCSubnets()
		g.Expect(instance).To(BeFalse())
		g.Expect(err).To(BeNil())
...
func TestCheckVPCSubnet(t *testing.T) {
	...
	t.Run("When GetVPCSubnetByName returns empty as vpc subnet doesnot exist", func(t *testing.T) {
		g := NewWithT(t)
		setup(t)
		t.Cleanup(teardown)
		...
		mockVPC.EXPECT().GetVPCSubnetByName(gomock.Any()).Return(nil, nil)
		instance, err := clusterScope.checkVPCSubnet("subnet1Name")
		g.Expect(instance).To(Equal(""))
		g.Expect(err).To(BeNil())
	})
	...
	t.Run("When GetVPCSubnetByName returns success", func(t *testing.T) {
		...
		instance, err := clusterScope.checkVPCSubnet("subnet1Name")
		g.Expect(instance).To(Equal(*subnet1Details.ID))
		g.Expect(err).To(BeNil())
	})
}
...
func TestCreateVPCSubnet(t *testing.T) {
	...
	t.Run("When VPCZonesForVPCRegion returns no vpczones", func(t *testing.T) {
		g := NewWithT(t)
		setup(t)
		t.Cleanup(teardown)
