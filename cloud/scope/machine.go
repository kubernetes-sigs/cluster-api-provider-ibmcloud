...
func (m *MachineScope) CreateMachine(ctx context.Context) (*vpcv1.Instance, error) { //nolint: gocyclo
	...
	if m.IBMVPCMachine.Spec.Image != nil {
		...
		log.Info("machine creation configured with existing image", "machineName", m.IBMVPCMachine.Name, "imageID", *imageID)
		...
		if m.IBMVPCMachine.Spec.CatalogOffering.OfferingCRN != nil {
			...
			log.Info("machine creation configured with catalog offering", "machineName", m.IBMVPCMachine.Name, "offeringCRN", *m.IBMVPCMachine.Spec.CatalogOffering.OfferingCRN)
			...
			log.Info("machine creation configured with catalog version", "machineName", m.IBMVPCMachine.Name, "versionCRN", *m.IBMVPCMachine.Spec.CatalogOffering.VersionCRN)
		} else {
		...
		if m.IBMVPCMachine.Spec.CatalogOffering.PlanCRN != nil {
			...
			log.Info("machine creation configured with catalog plan", "machineName", m.IBMVPCMachine.Name, "planCRN", *m.IBMVPCMachine.Spec.CatalogOffering.PlanCRN)
		}
...
func (m *MachineScope) configurePlacementTarget(ctx context.Context) (vpcv1.InstancePlacementTargetPrototypeIntf, error) {
	...
	if m.IBMVPCMachine.Spec.PlacementTarget.DedicatedHost != nil {
		...
		log.Info("machine creation configured with dedicated host placement", "machineName", m.IBMVPCMachine.Name, "dedicatedHostID", *dedicatedHostID)
...
func (m *MachineScope) volumeToVPCVolumeAttachment(ctx context.Context, volume *infrav1.VPCVolume) *vpcv1.VolumeAttachmentPrototypeInstanceByImageContext {
	...
	if volume.EncryptionKeyCRN != "" {
		...
		log.Info("machine creation configured with volumn encryption key", "machineName", m.IBMVPCMachine.Name, "encryptionKeyCRN", volume.EncryptionKeyCRN)
	}
	...
	for _, member := range poolMembers.Members {
		if target, ok := member.Target.(*vpcv1.LoadBalancerPoolMemberTarget); ok {
			...
			if *target.Address == *internalIP {
				log.Info("found existing load balancer pool member for machine", "machineName", m.IBMVPCMachine.Spec.Name, "internalIP", *internalIP, "poolID", *poolID, "loadBalancerID", *loadBalancerID)
